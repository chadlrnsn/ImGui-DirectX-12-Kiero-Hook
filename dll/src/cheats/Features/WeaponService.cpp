...
void WeaponService::SaveOriginalSettings(SDK::URGWeaponScript* weaponScript) {
    ...
    if (!ws) { LOG_ERROR("Failed to get weapon settings for saving originals"); return; }
...
void WeaponService::ApplyToSecondaryWeapon(SDK::URGWeaponScript* weaponScript, bool restore) {
    if (s_isEngineRifle && s_cachedEngineRifleScript && s_cachedEngineRifleScript->SecondaryWeaponModScript) {
