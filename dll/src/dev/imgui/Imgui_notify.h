...
namespace ImGui
{
	...
	NOTIFY_INLINE VOID RenderNotifications()
	{
		...
		for (auto i = 0; i < notifications.size(); i++)
		{
			...
			SetNextWindowPos(ImVec2(GetIO().DisplaySize.x - NOTIFY_PADDING_X, GetIO().DisplaySize.y - NOTIFY_PADDING_Y - height), ImGuiCond_Always, ImVec2(1.0f, 1.0f));
			Begin(window_name, NULL, NOTIFY_TOAST_FLAGS);

			// Here we render the toast content
			{
				PushTextWrapPos(GetIO().DisplaySize.x / 3.f); // We want to support multi-line text, this will wrap the text after 1/3 of the screen width
